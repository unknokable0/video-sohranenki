...
class SohrUpdateManager(private val context: Context) {
    ...
    fun isSignatureCompatible(apk: File): Boolean = runCatching {
        val currentDigests = signingDigests(context.packageManager.getPackageInfo(context.packageName, signingFlags()))
        val archiveInfo = context.packageManager.getPackageArchiveInfo(apk.absolutePath, signingFlags()) ?: return false
        val archiveDigests = signingDigests(archiveInfo)
        currentDigests.isNotEmpty() && archiveDigests.isNotEmpty() && currentDigests.any { it in archiveDigests }
    }.getOrDefault(false)
    ...
    private fun signingDigests(info: PackageInfo): Set<String> {
        val signatures = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            ...
            (if (signingInfo.hasMultipleSigners()) signingInfo.apkContentsSigners else signingInfo.signingCertificateHistory).toList()
        } else {
    ...
    fun canRequestInstall(): Boolean = Build.VERSION.SDK_INT < Build.VERSION_CODES.O || context.packageManager.canRequestPackageInstalls()
